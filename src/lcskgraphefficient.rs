...
pub fn lcskpp_graph(kmer_pos_vec: Vec<(u32, u32)>, kmer_path_vec: Vec<Vec<usize>>, paths: &Vec<Vec<usize>>, k: usize) {
    ...
    for (idx, &(x, y)) in kmer_pos_vec.iter().enumerate() {
        ...
        // this can be done in the kmer match finder
    ...
    for ev in events {
        ...
        if is_start {
            ...
            if ev.0 >= k {
                ...
                // we can just prev node in graph have to modify this *********
                println!("candidate location {} {}", ev.0 - k, ev.1 - 1);
                for path in &ev.4 {
                    if let Some(index_path) = paths[*path].iter().position(|r| r == &(ev.1 as usize)) {
                        if index_path > 0 {
                            let ev1_minus_1 = paths[*path][index_path - 1] as u32;
                            if let Ok(cont_idx) = kmer_pos_vec.binary_search(&(ev.0 - k, ev1_minus_1)) {
                                let prev_score = dp[cont_idx].0;
                                let candidate = (prev_score + 1, cont_idx as i32);
                                dp[p] = max(dp[p], candidate);
                                best_dp = max(best_dp, (dp[p].0, p as i32, *path));
                                println!("candidate location {} {}", ev.0 - k, ev1_minus_1);
